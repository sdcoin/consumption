...
{
    resize(850, 550);
    setWindowTitle(tr("ConsumptionBlockChain") + " - " + tr("Wallet"));
#ifndef Q_OS_MAC
...
void BitcoinGUI::createActions()
{
    ...
    sendCoinsAction->setToolTip(tr("Send coins to a ConsumptionBlockChain address"));
    ...
    aboutAction = new QAction(QIcon(":/icons/bitcoin"), tr("&About ConsumptionBlockChain"), this);
    aboutAction->setToolTip(tr("Show information about ConsumptionBlockChain"));
    ...
    optionsAction->setToolTip(tr("Modify configuration options for ConsumptionBlockChain"));
...
void BitcoinGUI::setClientModel(ClientModel *clientModel)
{
    ...
    if(clientModel)
    {
        ...
        if(clientModel->isTestNet())
        {
            ...
            if(trayIcon)
            {
                trayIcon->setToolTip(tr("ConsumptionBlockChain client") + QString(" ") + tr("[testnet]"));
...
void BitcoinGUI::createTrayIcon()
{
    ...
    trayIcon->setToolTip(tr("ConsumptionBlockChain client"));
...
void BitcoinGUI::setNumConnections(int count)
{
    ...
    labelConnectionsIcon->setToolTip(tr("%n active connection(s) to ConsumptionBlockChain network.", "", count));
}
...
void BitcoinGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
        ...
        if (nValidUrisFound)
            ...
            notificator->notify(Notificator::Warning, tr("URI handling"), tr("URI can not be parsed! This can be caused by an invalid ConsumptionBlockChain address or malformed URI parameters."));
    }
